...
describe 'ntp' do
  ...
  context 'on osfamily EL with default class options' do
    ...
    it {
      should contain_exec('mkdir_p-/etc/ntp/step-tickers').with({
        'command' => 'mkdir -p /etc/ntp/step-tickers',
        'unless'  => 'test -d /etc/ntp/step-tickers',
      })
    }
    ...
    it {
      should contain_file('step-tickers').with({
        ...
        'group'  => 'root',
        'mode'   => '0644',
        'require' => [ 'Package[ntp_package]', 'Common::Mkdir_p[/etc/ntp/step-tickers]' ],
      })
    }
